...
### Average Number of Ancestors (ANA)
 ...
 - **Description:** A measure of the generalization-specialization aspect of the design. Classes in a design which have one or more descendants exhibit this property of abstraction
 - **Interpreted as:** It counts the average number of classes from which a class inherits information, e.g., average of super classes


| Design Property| Acronym | Name | Description | Interpreted As|
|--|:--:|--|--|--|
| Encapsulation | DAM | Data Access Metric | Defined as the enclosing of data and behavior within a single construct. In object-oriented designs the property specifically refers to designing classes that prevent access to attribute declarations by defining them to be private, thus protecting the internal representation of the objects. | Interpreted as the average across all design classes with at least one attribute, of the ratio of non-public to total attributes in a class |
| Coupling | DCC | Direct Class Coupling | Defines the interdependency of an object on other objects in a design. It is a measure of the number of other objects that would have to be accessed by an object in order for that object to function correctly. | It is an average over all classes when applied to a design as a whole; a count of the number of distinct user-defined classes a class is coupled to by method parameter or attribute type. We exclude standard Java library classes from the computation |
